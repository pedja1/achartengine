...
public abstract class XYChart extends AbstractChart
{
    ...
    public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint)
    {
        ...
        int maxIconWidth = mRenderer.getYIcon(0d, 0).getWidth();
        int[] iconMargins = mRenderer.getIconMargins();
        int iconMarginLeft = iconMargins[1];
        int iconMarginRight = iconMargins[3];
        ...
        for (int i = 0; i < sLength; i++)
        {
            ...
            synchronized (series)
            {
                ...
                for (Entry<Double, Double> value : range.entrySet())
                {
                    ...
                    {
                        points.add((float) (iconMarginLeft * 2 + maxIconWidth + xPixelsPerUnit[scale] * (xValue - minX[scale])));
        ...
        if (showLabels || showGridX)
        {
            ...
            drawYIcons(allYLabels, canvas, paint, maxScaleNumber, left, right, bottom, yPixelsPerUnit,
                    minY);
            ...
            if (showLabels)
            {
                ...
                for (int i = 0; i < maxScaleNumber; i++)
                {
                    ...
                    Double[] yTextLabelLocations = mRenderer.getYIconLocations(i);
                    for (Double location : yTextLabelLocations)
                    {
                        if (minY[i] <= location && location <= maxY[i])
                        {
                            ...
                            Bitmap icon = mRenderer.getYIcon(location, i);
                            if (icon.getWidth() > maxIconWidth)
                            {
                                maxIconWidth = icon.getWidth();
                            }
                            ...
                            if (or == Orientation.HORIZONTAL)
                            {
                                if (axisAlign == Align.LEFT)
                                {
                                    if (showTickMarks)
                                    {
                                        canvas.drawLine(iconMarginLeft * 2 + maxIconWidth + getLabelLinePos(axisAlign), yLabel, iconMarginLeft * 2 + maxIconWidth, yLabel, paint);
                                    }
                                    drawIcon(canvas, icon, iconMarginLeft, yLabel - maxIconWidth / 2/* - mRenderer.getYLabelsVerticalPadding()*/,
                                            paint);
                                }
                                else
                                {
                                    if (showTickMarks)
                                    {
                                        canvas.drawLine(iconMarginRight * 2 + maxIconWidth, yLabel, iconMarginRight * 2 + maxIconWidth + getLabelLinePos(axisAlign), yLabel,
                                                paint);
                                    }
                                    drawIcon(canvas, icon, iconMarginRight, yLabel - maxIconWidth / 2/* - mRenderer.getYLabelsVerticalPadding()*/,
                                            paint);
                                }
                            ...
                            else
                            {
                                ...
                                drawIcon(canvas, icon, iconMarginRight + 10, yLabel - maxIconWidth / 2/* - mRenderer.getYLabelsVerticalPadding()*/,
                                        paint);
                                if (showCustomTextGridY)
                                {
                                    paint.setColor(mRenderer.getGridColor(i));
            ...
            drawXLabels(xLabels, mRenderer.getXTextLabelLocations(), canvas, paint, iconMarginLeft * 2 + maxIconWidth, top,
        ...
        {
            paint.setStrokeWidth(mRenderer.getAxisWidth());
            paint.setColor(mRenderer.getXAxisColor());
            canvas.drawLine(iconMarginLeft * 2 + maxIconWidth, bottom, right, bottom, paint);//draw x axes
            ...
            if (or == Orientation.HORIZONTAL)
            {
                canvas.drawLine(iconMarginLeft * 2 + maxIconWidth, top, iconMarginLeft * 2 + maxIconWidth, bottom, paint);
                if (rightAxis)
                {
                    canvas.drawLine(iconMarginRight * 2 + maxIconWidth, top, iconMarginRight * 2 + maxIconWidth, bottom, paint);
                }
            }
            else if (or == Orientation.VERTICAL)
            {
                                       ...
                                       Paint paint, List<Float> points, int seriesIndex, int startIndex)
    {
        ...
        { // there are more than one point
            ...
            {
                if (k == 2)
                { // decide whether to display first two points' values or
                    // not
     ...
     * @param paint  the paint to be used for drawing
     */
    protected void drawIcon(Canvas canvas, Bitmap bitmap, float x, float y, Paint paint)
    {
     ...
     *
     * @param allYIcons      the Y labels values
     * @param canvas         the canvas to paint to
     * @param paint          the paint to be used for drawing
     ...
     */
    protected void drawYIcons(Map<Integer, List<Double>> allYIcons, Canvas canvas, Paint paint,
                              int maxScaleNumber, int left, int right, int bottom, double[] yPixelsPerUnit, double[] minY)
    {
        ...
        {
            paint.setTextAlign(mRenderer.getYLabelsAlign(i));
            List<Double> yLabels = allYIcons.get(i);
            int length = yLabels.size();
            for (int j = 0; j < length; j++)
            {
                double label = yLabels.get(j);
                Align axisAlign = mRenderer.getYAxisAlign(i);
                boolean icon = mRenderer.getYIcon(label, i) != null;
                ...
                if (or == Orientation.HORIZONTAL)
                {
                    if (showLabels && !icon)
                    {
                        ...
                        if (axisAlign == Align.LEFT)
                        {
                            ...
                            drawIcon(canvas, mRenderer.getYIcon(label, 0),
